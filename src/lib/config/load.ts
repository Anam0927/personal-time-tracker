...
function parseConfig(config: Record<string, unknown>, logLevelOverride?: LogLevel): Config {
...
export async function loadConfig(
  ...
  if (!xdgConfig) {
    throw new Error("XDG_CONFIG_HOME is not set. Cannot determine config directory.")
  }
  ...
  const explorer = cosmiconfig("time-tracker", {
    searchPlaces: ["package.json", `.${MODULE_NAME}rc.json`, `${MODULE_NAME}.config.json`],
