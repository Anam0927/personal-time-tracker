...
const isCommanderBootstrapError = (error: unknown): error is CommanderBootstrapError => {
  return (
    typeof error === "object" &&
    error !== null &&
    "code" in error &&
...
const main = async (): Promise<void> => {
  try {
    ...
    if (!parseResult) {
      console.error('No command provided. Run "bun run cli -- help" to see available commands.')
      process.exit(1)
      return
    }
